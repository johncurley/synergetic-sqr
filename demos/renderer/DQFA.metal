...
// --- 1. SOVEREIGN ALGEBRAIC CORE (Integer Only) ---
...
struct DisplayCorner {
    ...
    static float drawIVM(float2 uv, float tick, float scale) {
        ...
        float d1 = abs(sin(dot(g, p1) + tick * 0.01));
        float d2 = abs(sin(dot(g, p2) + tick * 0.01));
        float d3 = abs(sin(dot(g, p3) + tick * 0.01));
...
kernel void renderDQFA_v1_5(
    ...
    float3 final_color = float3(0.05, 0.05, 0.08); // Baseline
