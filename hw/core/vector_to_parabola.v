    ...
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            ...
            if (dist_sq > 16'd576) begin 
                ...
                // Simplified for iCE40: base_energy - (dist_sq >> 1) approx.
                // More precise: pixel_out = base_energy - ((base_energy * dist_sq) / 576)
                // For the audit, we use the simple shift-based arch:
                pixel_out <= (base_energy > (dist_sq >> 1)) ? (base_energy - (dist_sq[7:0] >> 1)) : 8'd0; 
            end
        end
    end
endmodule
